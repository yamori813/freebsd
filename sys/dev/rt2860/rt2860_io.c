...
#define EEPROM_IN_FLASH 0x1f040000
#endif
...
uint16_t rt2860_io_eeprom_read(struct rt2860_softc *sc, uint16_t addr)
{
	...
	if (IS_SOC(sc)) {
		...
		uint8_t *eeprom = (uint8_t *)MIPS_PHYS_TO_KSEG1(EEPROM_IN_FLASH);
...
void rt2860_io_rf_load_defaults(struct rt2860_softc *sc)
{
	...
	for (i = 0; i < sizeof(rt3052_rf_default)/sizeof(REG_PAIR); i ++)
		rt2860_io_rf_write(sc, rt3052_rf_default[i].reg,
		    rt3052_rf_default[i].val);
#endif
}
...
int rt2860_io_rf_get_default(int reg)
{
	...
	for (i = 0; i < sizeof(rt3052_rf_default)/sizeof(REG_PAIR); i ++)
		if (rt3052_rf_default[i].reg == reg)
		    return rt3052_rf_default[i].val;
