/*	$OpenBSD: if_rt2860var.h,v 1.3 2009/03/26 20:17:27 damien Exp $	*/
...
struct rt2860_usb_rx_radiotap_header {
...
struct rt2860_usb_tx_radiotap_header {
...
struct rt2860_usb_tx_data {
	STAILQ_ENTRY(rt2860_usb_tx_data)	next;
...
STAILQ_HEAD(rt2860_usb_tx_data_head, rt2860_usb_tx_data);

struct rt2860_usb_node {
	struct ieee80211_node	ni;
...
#define RUN_NODE(ni)		((struct rt2860_usb_node *)(ni))

struct rt2860_usb_cmdq {
	void			*arg0;
	void			*arg1;
	void			(*func)(void *);
...
#define	RUN_VAP(vap)	((struct rt2860_usb_vap *)(vap))
...
struct rt2860_usb_endpoint_queue {
	struct rt2860_usb_tx_data		tx_data[RUN_TX_RING_COUNT];
	struct rt2860_usb_tx_data_head		tx_qh;
	struct rt2860_usb_tx_data_head		tx_fh;
	uint32_t			tx_nfree;
};
...
#define	RUN_RETRY	2
	...
	int				(*sc_srom_read)(struct rt2860_usb_softc *,
					    uint16_t, uint16_t *);
	...
	struct rt2860_usb_endpoint_queue	sc_epq[RUN_EP_QUEUES];

	struct task                     ratectl_task;
	struct usb_callout              ratectl_ch;
	uint8_t				ratectl_rt2860;
...
#define	RUN_CMDQ_MASQ	(RUN_CMDQ_MAX - 1)
	struct rt2860_usb_cmdq			cmdq[RUN_CMDQ_MAX];
...
#define	RUN_CMDQ_GO	1
	...
	union {
		struct rt2860_usb_rx_radiotap_header th;
		uint8_t	pad[64];
	}				sc_rxtapu;
#define sc_rxtap	sc_rxtapu.th

	union {
		struct rt2860_usb_tx_radiotap_header th;
		uint8_t	pad[64];
	}				sc_txtapu;
